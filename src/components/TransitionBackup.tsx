...
const StyledDiv = styled.div<StyledDivProps>`
  ${({ status, direction }) => (status === 'entering' && direction === 'right') && css`
    ...
    animation: ${rightEnter} ${duration}ms ease-in-out;
  `}

  ${({ status, direction }) => (status === 'exiting' && direction === 'right') && css`
    /* transform-origin: 0% 50%; */
    animation: ${leftExit} ${duration}ms ease-in-out;
  `}

  ${({ status, direction }) => (status === 'entering' && direction === 'left') && css`
    /* transform-origin: 100% 50%; */
    animation: ${leftEnter} ${duration}ms ease-in-out;
  `}

  ${({ status, direction }) => (status === 'exiting' && direction === 'left') && css`
    /* transform-origin: 100% 50%; */
    animation: ${rightExit} ${duration}ms ease-in-out;
  `}
`
            ...
            >
              {(status: string) => (
                <StyledDiv status={status} direction={context.direction}>
                  {console.log(context ,status, 'these are context and status')}
                  {children}
                </StyledDiv>
              )}
            </ReactTransition>
